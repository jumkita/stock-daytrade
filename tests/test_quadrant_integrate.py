...
def test_apply_quadrant_rank_mode_keeps_items(monkeypatch, tmp_path):
    """rank モードでは tech 不可でもシグナルを維持する。"""
    ...
    out, stats = apply_quadrant_to_backtest_items(items, {}, csv, mode="rank")
    assert stats.after == 1
    assert stats.mode == "rank"
    assert out[0]["pattern_name"] == "包み線"
...
def test_apply_quadrant_filters_low_ma(monkeypatch, tmp_path):
    ...
    out, stats = apply_quadrant_to_backtest_items(items, bulk, csv, min_score=0, mode="filter")
