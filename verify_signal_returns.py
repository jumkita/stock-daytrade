...
【前提】GitHub Actions は平日 JST 15:15 前後を目安に実行され、daily_buy_signals.json を上書きする。
        JSON の "updated" はその実行日時（UTC）、"items" はその日に買いシグナルが出た銘柄リスト。

【検証ロジック】
1. シグナル日: "updated" を JST に変換した「日付」をシグナル日とする（平日夕方の実行ならその日がシグナル日）。
2. エントリー: 各銘柄の "entry"（＝その日時点の終値に相当）を「引けで約定した買い価格」とする。
3. 利確: シグナル日当日の行を起点に、営業日カウントで N 本先の終値を利確とする（iloc[N]）。
   N=1 は「翌営業日の大引け」、N は既定 1（UI・CLI で変更可）。
...
def verify_returns(
    ...
    Args:
        ...
        holding_days: 利確までの営業日オフセット（1=シグナル日の翌営業日の終値）
